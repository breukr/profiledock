...
PLIST
if [[ "${PROFILEDOCK_CONTEXT_PREVIEW:-0}" == "1" ]]; then
    /usr/libexec/PlistBuddy -c "Set :CFBundleIdentifier $bundle_id" -c "Set :CFBundleName $bundle_name" -c "Set :CFBundleDisplayName $bundle_name" -c "Set :CFBundleVersion 147" -c "Set :CFBundleShortVersionString 1.4.0-preview.8" -c "Add :ProfileDockContextPreview bool true" "$bundle/Contents/Info.plist"
fi
