...
3. If the profile is already open, it keeps running while the copy is prepared. Quit and reopen it when you are ready to use the new icon; there is no forced restart. To pin its app, use **Show Dock app** and drag that app to the Dock, or choose **Options → Keep in Dock** on its running icon.
...
5. To return to the usual launch mode, close the profile and turn off **Native macOS Dock icon** in the profile editor. This moves the generated app to the Trash and keeps its profile data. Remove its old Dock pin yourself if necessary.
