...
First inspect the running/built ProfileDock version and the actual current UI. The target is **ProfileDock 1.4.0, build 144 or later**, including context tagging and optional native macOS Dock icons. Use the implemented behavior, screenshots, `docs/CONTEXT.md`, and `docs/NATIVE-DOCK.md` as the source of truth. If the available build differs, report that before recording or making feature claims. Do not describe a local build or open PR as a published release.
...
- ProfileDock itself has a clearly named **Settings** sidebar tab with separate Dock and menu-bar visibility switches and startup controls. **Appearance** is for visual options. When its Dock preference is off, a temporary Dock icon appears while Settings is open to support the native top-left app menu; it hides again when Settings closes. Show the **ProfileDock** menu with distinct ProfileDock and ChatGPT/Codex update commands in a longer settings cut if useful. Do not confuse this companion visibility setting with each profile's experimental Dock icon.
