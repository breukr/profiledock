...
Codex usage requests go directly to OpenAI and reject redirects. Claude usage requests read only the current macOS user’s `Claude Code-credentials` Keychain item and send its access token directly to `api.anthropic.com` over HTTPS, rejecting redirects. The token is never logged, copied to preferences, refreshed or changed by ProfileDock. Account and organization identifiers are hashed for in-memory identity checks. Optional saved-reset tracking opens `claude.ai` in ProfileDock’s own WebKit session after you select Connect. Its cookies stay in the app’s WebKit data store, are not imported from other apps, and are removed on Disconnect. Read-only requests verify organization access before reading reset grants. No reset is consumed. Update checks and selected downloads use OpenAI's `persistent.oaistatic.com` update feed. Downloads must pass its Ed25519 signature and the installed app must have OpenAI's Apple developer signature. No update requires signing into a third-party service.
